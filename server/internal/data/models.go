...
func (u User) Insert(user User) (int, error) {
	...
	err = db.QueryRowContext(
		ctx, stmt, user.FirstName, user.LastName, user.Email, hashedPassword, time.Now(), time.Now(),
	).Scan(&newID)
	if err != nil {
		return 0, err
	}
...
func (u User) Update() error {
	...
	_, err := db.ExecContext(
		ctx, stmt, u.Email, u.FirstName, u.LastName, time.Now(), u.ID,
	)
	if err != nil {
		return err
	}
...
func (u User) PasswordMatches(plainPass string) (bool, error) {
	err := bcrypt.CompareHashAndPassword([]byte(u.Password), []byte(plainPass))
	if err != nil {
		switch {
